...
func (db cliDb) SetCommit(ctx context.Context, quotaOwner string, repoId uint64, c commit.Commit) (err error) {
	...
	// can be listed. Note that the ids are the ones of this db, i.e. on a
	// server they are the server ids.
...
func (db cliDb) GetLatestCommitByLocalId(ctx context.Context, repoId uint64, L uint64) (c commit.Commit, isNotFoundErr bool, err error) {
	var latestV uint64
	err = db.s.QueryRow(ctx, `
	...
	c, err = readIntoStruct[commit.Commit](r, closeR)
	return
}

func (db cliDb) GetCommitVersionByLocalId(ctx context.Context, repoId uint64, L uint64, v uint64) (c commit.Commit, isNotFoundErr bool, err error) {
...
func (db cliDb) GetLatestCommitByServerId(ctx context.Context, repoId uint64, ServerL uint64) (c commit.Commit, isNotFoundErr bool, err error) {
	var commitId uint64
	err = db.s.QueryRow(ctx, `
...
func (db cliDb) GetCommitVersionByServerId(ctx context.Context, repoId uint64, ServerL uint64, ServerV uint64) (c commit.Commit, isNotFoundErr bool, err error) {
...
func (db cliDb) GetCommitChildren(ctx context.Context, repoId uint64,
	commitL uint64, commitV uint64, maxRowsToRead int) (
	children []commit.LocalId, childrenVersions []uint64, err error) {
	// One extra row is read to detect that there are more children than
