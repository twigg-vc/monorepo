...
func (db cliDb) GetTreeData(ctx context.Context, repoId uint64, treePath string, v uint64) (td treev.TreeDataV, isNotFoundErr bool, err error) {
	...
	td, err = readIntoStruct[treev.TreeDataV](r, closeR)
	return
}

func (db cliDb) GetTreeBlob(ctx context.Context, repoId uint64, treePath string, v uint64) (r io.Reader, closeR func(), isNotFoundErr bool, err error) {
...
func (db cliDb) SetTreeData(ctx context.Context, quotaOwner string, repoId uint64, treePath string, td treev.TreeDataV) (uint64, error) {
	return db.setBlob(ctx, quotaOwner, treeDataBlobsIdPrefix, treeDataBlobId(repoId, treePath), structWriterTo(td))
}
