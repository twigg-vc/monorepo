...
import (
	...
	"monorepo/twigg/diff/epiclabs-io/linereader"
)
...
func addConflictMarkers(lines, conflictA, conflictB []string, labelA, labelB string) []string {
	if IsTesting {
		lines = append(lines, fmt.Sprintf("%s%s", ConflictStart, labelA))
		lines = append(lines, conflictA...)
		lines = append(lines, ConflictMid)
		lines = append(lines, conflictB...)
		lines = append(lines, fmt.Sprintf("%s%s", ConflictEnd, labelB))
	} else {
		lines = append(lines, fmt.Sprintf("%s%s", ConflictStart, labelA))
		lines = append(lines, conflictA...)
		lines = append(lines, ConflictMid)
